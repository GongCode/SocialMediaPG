...
class KDTree
{
  ...
  private:
    ...
    /**
     * Calculates distance^2 to avoid unnecessary sqrt operation.
     *
     * @param first  The first point to calculate distance from
     * @param second The second point to calculate distance to
     * @return Sum((first[i] - second[i])^2)
     */
     ...
     *
     * @see Wikipedia <https://en.wikipedia.org/wiki/Quickselect>
     *
     * @param points The points vector to partition
     * @param dim Current dimension
     * @param left Left index
     * @param right Right index
     * @param pivotIndex Arbitrary pivot index
     * @return The new pivot index
     */
    int quickPartition(vector<Point<Dim>>& points, int dim, int left, int right, int pivotIndex);
     ...
     * @param left Left index
     * @param right Right index
     * @param k Arbitrary pivot index
     * @return The new pivot index
     */
    int quickSelect(vector<Point<Dim>>& points, int dim, int left, int right, int k);

    void clear(KDTreeNode* subRoot);

    /**
     * Builds a tree node from given points recursively.
     *
     * @param points The points vector to build tree from
     * @param dim Current dimension
     * @param The starting index
     * @param The ending index
     * @return The new subroot node
     */
    KDTreeNode *build(std::vector<Point<Dim>> &points, int dim, int start, int end);

    KDTreeNode *copy(const KDTreeNode * subRoot);

    /**
     * Internal function for public findNearesteNeighbor function
     *
     * @param node Node to start from
     * @param query Target point
     * @param Current dimension
...
//     int partition(const vector<Point<Dim>>& newPoints, int pivotPoint, int left, int right, int dim);

    
//     Point<Dim> select(const vector<Point<Dim>>& newPoints, int left, int right, int dim, int k);

    
//     void createTree(const vector<Point<Dim>>& newPoints,int left, int right, int dim, KDTreeNode *& cur);
// };
